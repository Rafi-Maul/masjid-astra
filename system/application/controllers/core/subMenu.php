<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class SubMenu extends MyController {
	
		public function __construct() {
			log_message('DEBUG', 'core::SubMenu Class Initialized');
			parent::__construct();
		}
		...
		public function index() {
			...
			$sql = "
				   ...
				   b.menu,
				   c.*,
				   d.id_dd_sub_menus AS id_dd_tabs
				   FROM
				   dd_moduls a
				   ...
				   LEFT JOIN (
						...
						GROUP BY
						id_dd_sub_menus
				   ) d ON c.id_dd_sub_menus = d.id_dd_sub_menus
				   WHERE
				   {$sql_plus}		
				   ORDER BY
				   a.order_number,
			...
			try {
				$arr_submenu =& $this->db->createPaging($page, $this->getListPerPage(), $sql, $params);
			} catch (Exception $e) {
			...
			$data['arr_submenu'] = $arr_submenu;
			$this->load->viewPage('core/list_submenu', $data);				
		}	
		
		public function add() {
			$this->password->getUrlAccess('/core/subMenu', 'tambah');
			...
			$this->load->viewPage('core/list_submenu_add', $data);
		}
		...
		public function addAct() {
			...
			try {
				...
				$sub_menu 	  = $this->getVar('sub_menu', TRUE);				
				$order_number = $this->getVar('order_number', TRUE);
				$note 		  = $this->getVar('note', FALSE);
				
				$insert					= array();
				$insert['id_dd_menus']	= $id_dd_menus;								
				$insert['sub_menu']		= $sub_menu;
				$insert['order_number'] = $order_number;
				$insert['note']			= $note;
				$this->db->insert('dd_sub_menus', $insert);	
				...
				echo "Tidak dapat menambah data sub menu\n\nPenjelasan Teknis:\n{$e->getMessage()}";
			}
		}	
		
		public function edit($id_dd_sub_menus, $page, $filter, $keyword = '') {
			$this->password->getUrlAccess('/core/subMenu', 'edit');
			...
			$sub_menu = $this->db->getRow($sql, array($id_dd_sub_menus));
            ...
            $arr_menus =& $this->db->getRows($sql, array($sub_menu['id_dd_moduls']));
			...
			$data['filter']		= $filter;
			$data['keyword']	= $keyword;			
			$this->load->viewPage('core/list_submenu_edit', $data);
		}	
		...
		public function editAct() {
			...
			try {
				...
				$update					= array();
				$update['id_dd_menus']	= $id_dd_menus;
				$update['sub_menu']		= $sub_menu;
				$update['order_number'] = $order_number;
				$update['note']			= $note;
				$this->db->update('dd_sub_menus', $update, 'id_dd_sub_menus = ?', array($id_dd_sub_menus));
				...
				$this->db->endTrans(false);
				echo "Tidak dapat memperbaharui data sub menu\n\nPenjelasan Teknis:\n{$e->getMessage()}";
			}
		}	
		
		public function del() {
			$this->password->getUrlAccess('/core/subMenu', 'hapus');
			
			try {
				$this->db->beginTrans();
				
				$arr_id_dd_sub_menus = $this->getVar('id_list', TRUE);
				
				$where_delete = str_repeat('?,', count($arr_id_dd_sub_menus));
				$where_delete = 'id_dd_sub_menus IN (' . substr($where_delete, 0, -1) . ')';
				
				$this->db->delete('dd_sub_menus', $where_delete, $arr_id_dd_sub_menus);				
				
				$this->db->endTrans();
			} catch (Exception $e) {
				$this->db->endTrans(false);
				$this->session->set_flashdata('s4b_error', "Tidak dapat menghapus data sub menu\n\nPenjelasan Teknis:\n{$e->getMessage()}");
			}
			
			redirect('/core/subMenu');
		}	
		
		public function report($filter = 1, $keyword = '') {
			$this->password->getUrlAccess('/core/subMenu', 'cetak');
			...
			$this->report->SetReportMainTitle('LAPORAN SUB MENU');
...
/* Location: ./system/application/controllers/core/subMenu.php */

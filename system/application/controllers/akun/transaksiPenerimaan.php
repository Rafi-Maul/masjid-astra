<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class TransaksiPenerimaan extends MyController {
		
		public function __construct() {
			log_message('DEBUG', 'akun::transaksiPenerimaan Class Initialized');
			parent::__construct();
		}		
		...
		public function index() {
			...
			try {
				...
				redirect('/akun/transaksiPenerimaan');
				return;
			}
			...
			$this->load->viewPage('akun/list_transaksi_penerimaan', $data);
		}		
		
		public function add() {
			$this->password->getUrlAccess('/akun/transaksiPenerimaan', 'tambah');
			...
			$this->load->viewPage('/akun/list_transaksi_penerimaan_add', $data);
		}
		...
		public function addAct() {
			...
			try {
				...
				$kode_jurnal = $this->db->getField('kode', 'SELECT kode FROM akun.akdd_kodifikasi_jurnal WHERE id_akdd_kodifikasi_jurnal = 1');
				
				$tahun = substr($tanggal, 0, 4);
				$bulan = substr($tanggal, 5, 2);
				$no_bukti = intval($this->db->getField('no_bukti', "SELECT SUBSTRING(no_bukti from 9 for 8) AS no_bukti FROM akun.akmt_jurnal WHERE SUBSTRING(no_bukti from 1 for 2) = ? AND DATE_PART('MONTH', tanggal) = ? AND DATE_PART('YEAR', tanggal) = ? ORDER BY no_bukti DESC, tanggal DESC OFFSET 0 LIMIT 1", array($kode_jurnal, intval($bulan), $tahun)));				
		...
		public function edit($id_transaksi, $page, $filter, $keyword = '') {
		    $this->password->getUrlAccess('/akun/transaksiPenerimaan', 'edit');
			...
			$data['filter']		    = $filter;
			$data['keyword']	    = $keyword;
			$this->load->viewPage('/akun/list_transaksi_penerimaan_edit', $data);		    
		}
		...
		public function del() {
			$this->password->getUrlAccess('/akun/transaksiPenerimaan', 'hapus');
			...
			try {
				...
				$this->db->endTrans(false);
				$this->session->set_flashdata('s4b_error', "Tidak dapat menghapus data transaksi penerimaan\n\nPenjelasan Teknis:\n{$e->getMessage()}");
			}
		...
		public function report($filter = 1, $keyword = '') {
			$this->password->getUrlAccess('/akun/transaksiPenerimaan', 'cetak');
			...
			$this->report->SetReportMainTitle('LAPORAN JURNAL PENERIMAAN');
...
/* End of file transaksiPenerimaan.php */
/* Location: ./system/application/controllers/akun/transaksiPenerimaan.php */	

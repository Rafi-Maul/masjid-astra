<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class GuruTpa extends MyController {
		const TIPE 		= 3;
		const SELF_URL	= 'guruTpa';
	
		public function __construct() {
			log_message('DEBUG', 'basic::GuruTpa Class Initialized');
			parent::__construct();
		}
		...
		public function add() {
			$this->password->getUrlAccess('/basic/guruTpa', 'tambah');				
						
			$data		  	= array();
			$data['title']	= 'Tambah Guru TPA';
			$data['tipe']	= self::SELF_URL;
		...
		public function addAct() {
			...
			try {
				...
				echo "Tidak dapat menambah data guru tpa\n\nPenjelasan Teknis:\n{$e->getMessage()}";
			}
		}	

		public function edit($id_pihak_penerima, $page, $filter, $keyword = '') {
			$this->password->getUrlAccess('/basic/guruTpa', 'edit');
		...
		public function editAct() {
			...
			try {
				...
				$this->db->endTrans(false);
				echo "Tidak dapat memperbaharui data guru tpa\n\nPenjelasan Teknis:\n{$e->getMessage()}";
			}
		}		
		
		public function del() {
			$this->password->getUrlAccess('/basic/guruTpa', 'hapus');
			...
			try {
				...
				$this->db->endTrans(false);
				$this->session->set_flashdata('s4b_error', "Tidak dapat menghapus data guru tpa\n\nPenjelasan Teknis:\n{$e->getMessage()}");
			}
			
			redirect('/basic/guruTpa');
		}		
		
		public function report($filter = 1, $keyword = '') {
			$this->password->getUrlAccess('/basic/guruTpa', 'cetak');
			...
			$this->report->SetReportMainTitle('LAPORAN GURU TPA');
...
/* End of file guruTpa.php */
/* Location: ./system/application/controllers/basic/guruTpa.php */

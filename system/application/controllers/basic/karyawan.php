<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class karyawan extends MyController {
		const TIPE 		= 2;
		const SELF_URL	= 'karyawan';
	
		public function __construct() {
			log_message('DEBUG', 'basic::karyawan Class Initialized');
			parent::__construct();
		}
		...
		public function add() {
			$this->password->getUrlAccess('/basic/karyawan', 'tambah');				
		...
		public function addAct() {
			...
			try {
				...
				echo "Tidak dapat menambah data karyawan\n\nPenjelasan Teknis:\n{$e->getMessage()}";
			}
		}	

		public function edit($id_pihak_penerima, $page, $filter, $keyword = '') {
			$this->password->getUrlAccess('/basic/karyawan', 'edit');
		...
		public function editAct() {
			...
			try {
				...
				$this->db->endTrans(false);
				echo "Tidak dapat memperbaharui data karyawan\n\nPenjelasan Teknis:\n{$e->getMessage()}";
			}
		}		
		
		public function del() {
			$this->password->getUrlAccess('/basic/karyawan', 'hapus');
			...
			try {
				...
				$this->db->endTrans(false);
				$this->session->set_flashdata('s4b_error', "Tidak dapat menghapus data karyawan\n\nPenjelasan Teknis:\n{$e->getMessage()}");
			}
			
			redirect('/basic/karyawan');
		}		
		
		public function report($filter = 1, $keyword = '') {
			$this->password->getUrlAccess('/basic/karyawan', 'cetak');
			...
			$this->report->SetReportMainTitle('LAPORAN KARYAWAN');
...
/* Location: ./system/application/controllers/basic/karyawan.php */

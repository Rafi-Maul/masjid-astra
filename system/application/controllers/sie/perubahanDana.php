<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class PerubahanDana extends MyController {
		...
		public function __construct() {
			log_message('DEBUG', 'sie::PerubahanDana Class Initialized');
			parent::__construct();
		}
		...
		public function index() {
			...
			$this->load->viewPage('sie/list_perubahan_dana', $data);									
		}
		
		public function level() {
			$this->password->getUrlAccess('/sie/perubahanDana', 'cetak');
			...
			$this->report->SetReportMainTitle('LAPORAN PERUBAHAN DANA PER LEVEL PER PERIODE');
			...
			foreach ($arr_coa as $coa => $value) {
				...
				if (is_numeric($value['saldo'])) {
					if ($value['code'] == 4) {
						$penerimaan += $value['saldo'] * ($value['acc_type'] == 'd' ? 1 : -1);
					} else {
						$pengeluaran += $value['saldo'] * ($value['acc_type'] == 'd' ? 1 : -1);
					}
				} 
				
				if (($prev_code == 4) && ($value['code'] != 4)) {
					$this->report->SetX(LEFT_MARGIN);
					$this->report->Cell($left_width, DEFAULT_LN, 'TOTAL PENERIMAAN', 'TB', 0);
					if ($penerimaan >= 0)
						$this->report->Cell(0, DEFAULT_LN, numFormat($penerimaan), 'TB', 1, 'R');
					else
						$this->report->Cell(0, DEFAULT_LN, '(' . numFormat(abs($penerimaan)) . ')', 'TB', 1, 'R');
				}
			...
			$this->report->SetX(LEFT_MARGIN);
			$this->report->Cell($left_width, DEFAULT_LN, 'TOTAL PENYALURAN & PENGGUNAAN', 'TB', 0);
			if ($pengeluaran >= 0)
				$this->report->Cell(0, DEFAULT_LN, numFormat($pengeluaran), 'T', 1, 'R');
			else
				$this->report->Cell(0, DEFAULT_LN, '(' . numFormat(abs($pengeluaran)) . ')', 'TB', 1, 'R');
				
			$this->report->SetX(LEFT_MARGIN);
			$this->report->Cell($left_width, DEFAULT_LN, 'KENAIKAN/PENURUNAN SALDO DANA', 'TB', 0);
			$perubahan = $penerimaan - $pengeluaran;
			if ($perubahan >= 0)
				$this->report->Cell(0, DEFAULT_LN, numFormat($perubahan), 'TB', 1, 'R');
			else
				$this->report->Cell(0, DEFAULT_LN, '(' . numFormat(abs($perubahan)) . ')', 'TB', 1, 'R');
		...
		private function levelExcel($month, $year, $level_coa) {
			...
			$objExcel->getActiveSheet()		
					 ->setCellValue('A1', 'LAPORAN PERUBAHAN DANA PER LEVEL PER PERIODE')
					 ->setCellValue('A3', 'TANGGAL CETAK')
					 ->setCellValue('A4', 'PERIODE')
					 ->setCellValue('B3', ':')
					 ->setCellValue('B4', ':')
					 ->setCellValue('C3', strtoupper(getCurrentDate(FALSE)))
					 ->setCellValue('C4', (empty($month) ? $year : strtoupper(getMonthString($month) . ', ' . $year)))
					 ->setCellValue('A5', 'KODE PERKIRAAN')
					 ->setCellValue('E5', 'NOMINAL (Rp.)')
					 ->setTitle('Report - Aktivitas');			
			...
			foreach ($arr_coa as $coa => $value) {
				...
				if (($prev_code == 4) && ($value['code'] != 4)) {
					$objExcel->getActiveSheet()
							 ->setCellValue('A' . $row, 'TOTAL PENERIMAAN')
							 ->setCellValueExplicit('E' . $row++, $penerimaan, PHPExcel_Cell_DataType::TYPE_NUMERIC);
				}
				...
				if ($value['level'] <= $level_coa) {
					$objExcel->getActiveSheet()
							 ...
							 ->setCellValueExplicit('E' . $row++, $saldo, PHPExcel_Cell_DataType::TYPE_NUMERIC);
				}
			...
			$objExcel->getActiveSheet()
					 ->setCellValue('A' . $row, 'TOTAL PENYALURAN & PENGGUNAAN')
					 ->setCellValueExplicit('E' . $row++, $pengeluaran, PHPExcel_Cell_DataType::TYPE_NUMERIC);

			$perubahan = $penerimaan - $pengeluaran;
			$objExcel->getActiveSheet()
					 ->setCellValue('A' . $row, 'KENAIKAN/PENURUNAN SALDO DANA')
					 ->setCellValueExplicit('E' . $row++, $perubahan, PHPExcel_Cell_DataType::TYPE_NUMERIC);
		...
		public function template() {
			...
			$this->report->SetLogo($this->getImgFolder() . '/logo.jpg', 17);
			$this->report->SetReportMainTitle('LAPORAN PERUBAHAN DANA');
			...
			foreach ($arr_templates as $key => $template) {
				$id_akdd_perubahan_dana 	= $template['id_akdd_perubahan_dana'];
				$id_akdd_perubahan_dana_ref = $template['id_akdd_perubahan_dana_ref'];
				...
				// Harus dikalibrasi dalam menampilkan karena saldo dana (pasiva) bersaldo normal kredit...
				if ($nominal <= 0) {
			...
			$this->report->SetX(LEFT_MARGIN);
			//$this->report->SetFillColor(230, 230, 230);
			$this->report->Cell($left_width - INDENT_LN, DEFAULT_LN, 'JUMLAH SALDO DANA', 'TB', 0, 'L');
			//$this->report->SetFillColor(245, 245, 245);
			if ($total <= 0) {
		...
		private function templateExcel($month, $year) {
			...
			$objExcel->getActiveSheet()		
					 ->setCellValue('A1', 'LAPORAN PERUBAHAN DANA')
					 ->setCellValue('A3', 'TANGGAL CETAK')
					 ->setCellValue('A4', 'PERIODE')
					 ->setCellValue('B3', ':')
					 ->setCellValue('B4', ':')
			...
			foreach ($arr_templates as $key => $template) {
				...
				$objExcel->getActiveSheet()
						 ...
						 ->setCellValueExplicit('E' . $row++, $nominal, PHPExcel_Cell_DataType::TYPE_NUMERIC);
			...
			$objExcel->getActiveSheet()
					 ->setCellValue('A' . $row, 'JUMLAH SALDO DANA')
					 ->setCellValueExplicit('E' . $row++, $total, PHPExcel_Cell_DataType::TYPE_NUMERIC);
...
/* Location: ./system/application/controllers/sie/perubahanDana.php */		

	...
	class LapBulanan extends MyController {
		...
		public function kas() {
			...
			$coa = $this->db->getRow("
									 ...
									 ", array($id_akdd_detail_coa, $year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT) . '-01', $id_akdd_detail_coa));

			/****************************  END SQL ****************************/
			
			if ($kode_format != 1) {
				$this->kasExcel($month, $year, $id_akdd_detail_coa, $coa);
				exit(0);
			} 									
			...
			$sql = 	"
					...
					OFFSET 0
					";
			$awal =	$this->db->getField('awal', $sql, array($id_akdd_detail_coa, $year, $month));
			...
			$sql =	"
					...
					AND DATE_PART('MONTH', tanggal) = ?
					";
			$total	= $this->db->getRow($sql, array($id_akdd_detail_coa, $year, $month));
			...
			$sql =	"
					...
					AND DATE_PART('YEAR', tanggal) = ?
					AND DATE_PART('MONTH', tanggal) = ?
					AND id_akdd_detail_coa = ?
					ORDER BY
					(CASE WHEN mutasi_debet > 0 THEN 0 ELSE 1 END),
					tanggal
					";
			$arr_lap =& $this->db->getRows($sql, array($year, $month, $id_akdd_detail_coa));
			...
			$sql = 	"
					...
					OFFSET 0
					";
			$akhir = $this->db->getField('akhir', $sql, array($id_akdd_detail_coa, $year, $month));			
		...
		private function kasExcel($month, $year, $id_akdd_detail_coa, $coa) {
			$this->load->helper('date');
			$this->load->helper('number');
			...
			$objExcel->getActiveSheet()		
					 ...
					 ->setCellValue('C5', strtoupper($coa['uraian']))

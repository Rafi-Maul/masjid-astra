	...
	class Closing {
		...
		public function generate($throw_error = false) {
			...
			try {						
				...
				// Sistem kunci harus dibuka dahulu.
				$this->setUnlock($mem_obj);
		...
		private function setUnlock(&$mem_obj) {
			try {
		...
		private function close($year, $month) {
			...
			if ($this->needProcess($year, $month)) {
				if (!$this->deleted) {
					$this->ci->db->delete('akun.akmt_buku_besar', 
										  'tanggal >= ? AND id_akmt_periode NOT IN (SELECT id_akmt_periode FROM akmt_periode WHERE ((bulan = 0 AND flag_temp = 2) OR (bulan = 0 AND tahun = ?)))', 
										  array("{$year}-{$month}-01", $year));
					
					$this->ci->db->resetSequence('akun.akmt_buku_besar',
												 'id_akmt_buku_besar',
												 'akmt_buku_besar_id_akmt_buku_besar_seq');

					$this->ci->db->delete('akun.akmt_periode', 
										  '((tahun = ? AND bulan >= ?) OR tahun > ?) AND id_akmt_periode NOT IN (SELECT id_akmt_periode FROM akmt_periode WHERE bulan = 0 AND flag_temp = 2)', 
										  array($year, $month, $year));
												 
					$this->ci->db->resetSequence('akun.akmt_periode',
												 'id_akmt_periode',
												 'akmt_periode_id_akmt_periode_seq');
					$this->deleted = true;												 
				}
			} else
				return;
			 ...
			 ******************************************************************* 
			 * 27-06-2010, berdasarkan permintaan dari Astra maka data yang
			 * diproses adalah yang sudah disetujui saja. Dengan demikian maka
			 * filter yang digunakan menjadi flag_temp = 2 saja. Nilai
			 * sebelumnya adalah flag_temp IN (1, 2) yang artinya semua jurnal
			 * termasuk yang belum disetujui dapat diperhitungkan dalam proses
			 * saldo akhir.
			 ******************************************************************* 
			 */ 
			...
			$insert['flag_temp'] = 1;
			$this->ci->db->insert('akmt_periode', $insert);
			...
			if ($month == 12) {
				...
				$arr_naik_turun =& $this->ci->db->getRows('
														  ...
														  INNER JOIN akun.akdd_detail_coa c ON b.id_akdd_detail_coa = c.id_akdd_detail_coa
														  WHERE
														  a.binary_code BETWEEN 16 AND 128
														  ORDER BY
														  a.binary_code DESC
														  ');
			}			
			...
			foreach ($arr_trans as $trans) {
				...
				// Tidak ada perhitungan zakat di masjid. Skip
				// Melakukan perhitungan 1/8 penerimaan zakat untuk dana amil.
				// if (in_array($trans['id_akdd_detail_coa'], $this->ci->getIDPenerimaanZakat())) {
				// 	if ($trans['jumlah'] < 0)
				// 		throw new Exception('Jurnal penerimaan zakat no. bukti ' . $trans['no_bukti'] . ' tidak valid !');
				// 	if (!isset($total_pendapatan_zakat[$trans['id_akdd_detail_coa']]))
				// 		$total_pendapatan_zakat[$trans['id_akdd_detail_coa']] = 0;
				// 	$total_pendapatan_zakat[$trans['id_akdd_detail_coa']] += ($trans['jumlah'] * ($trans['flag_position'] == 'd' ? 1 : -1));	
				// }
			...
			$kodifikasi_jurnal 	= $this->ci->db->getField('kode', 'SELECT kode FROM akdd_kodifikasi_jurnal WHERE id_akdd_kodifikasi_jurnal = ?', array(7));
			...
			$last_closing 		= $this->ci->db->getField('no_bukti', 'SELECT no_bukti FROM akmt_jurnal WHERE no_bukti LIKE ? ORDER BY no_bukti DESC LIMIT 1', array("{$kodifikasi_jurnal}{$year}{$month_pad}%"));
			$prefix_no_bukti 	= "{$kodifikasi_jurnal}{$year}{$month_pad}";
			
			$no_bukti_numbering = intval(substr($last_closing, -8));
						
			// Menjumlah semua penerimaan zakat kas dan bank.
			//$total_pendapatan_zakat = array_sum($total_pendapatan_zakat) * (-1);			
			//$total_pendapatan_zakat = array_sum($total_pendapatan_zakat);			
			
			// Menghitung bagian amil dari zakat tersebut.
			//$bagian_amil_zakat = floatval(bcdiv(strval($total_pendapatan_zakat), '8.00', 2));
			
			// Merubah kode-kode akun yang berhubungan dengan pembagian dana zakat ke dana amil.
			//if ($bagian_amil_zakat != 0) {
				// Penerimaan Dana Amil Dari Dana Zakat.
				//$id_coa_penerimaan_amil_zakat = 72;				
				
			// 	$insert 					  = array();
			// 	$insert['id_akmt_periode'] 	  = $id_akmt_periode;
			// 	$insert['id_akdd_detail_coa'] = $id_coa_penerimaan_amil_zakat;
			// 	$insert['no_bukti'] 		  = $prefix_no_bukti . str_pad(++$no_bukti_numbering, 8, '0', STR_PAD_LEFT);
			// 	$insert['tanggal'] 			  = $tanggal;
			// 	$insert['keterangan'] 		  = 'Pembagian 1/8 penerimaan zakat ke dana amil';
			// 	$insert['awal'] 			  = $arr_coa[$id_coa_penerimaan_amil_zakat];
			// 	$insert['mutasi_debet']   	  = 0;
			// 	$insert['mutasi_kredit']  	  = ($bagian_amil_zakat * -1);
			// 	$insert['akhir'] 			  = $arr_coa[$id_coa_penerimaan_amil_zakat] + $bagian_amil_zakat;
			// 	$this->ci->db->insert('akmt_buku_besar', $insert);
				
			// 	// Bagian Dana Amil Dari Dana Zakat.
			// 	$id_coa_pengeluaran_zakat_amil = 73;
				
			// 	$insert 					  = array();
			// 	$insert['id_akmt_periode'] 	  = $id_akmt_periode;
			// 	$insert['id_akdd_detail_coa'] = $id_coa_pengeluaran_zakat_amil;
			// 	$insert['no_bukti'] 		  = $prefix_no_bukti . str_pad(++$no_bukti_numbering, 8, '0', STR_PAD_LEFT);
			// 	$insert['tanggal'] 			  = $tanggal;
			// 	$insert['keterangan'] 		  = 'Pembagian 1/8 penerimaan zakat ke dana amil';
			// 	$insert['awal'] 			  = $arr_coa[$id_coa_pengeluaran_zakat_amil];
			// 	$insert['mutasi_debet']   	  = ($bagian_amil_zakat * -1);
			// 	$insert['mutasi_kredit']  	  = 0;
			// 	$insert['akhir'] 			  = $arr_coa[$id_coa_pengeluaran_zakat_amil] + ($bagian_amil_zakat * -1);
			// 	$this->ci->db->insert('akmt_buku_besar', $insert);				
			// }
			...
			foreach ($arr_naik_turun as $naik_turun) {
				...
				// // Masukan pembagian amil sehingga menambahkan kenaikan dana amil.
				// if ($naik_turun['binary_code'] == 64) {
				// 	$perubahan_dana 			  += $bagian_amil_zakat;
				// 	$arr_coa[$id_akdd_detail_coa] += $bagian_amil_zakat;
				// }
				
				// // Masukan pengurangan zakat untuk amil sehingga mengurangi kenaikan dana zakat.
				// if ($naik_turun['binary_code'] == 16) {
				// 	$perubahan_dana 			  -= $bagian_amil_zakat;
				// 	$arr_coa[$id_akdd_detail_coa] -= $bagian_amil_zakat;
				// }
				
				// Saldo Zakat, Infaq & Sadaqoh dan Amil tidak dibutuhkan saat ini untuk perhitungan Kenaikan/Penurunan.
			...
			$this->ci->db->update('akun.akmt_jurnal', 
								  $update, 
								  "DATE_PART('MONTH', tanggal) = ? AND DATE_PART('YEAR', tanggal) = ? AND flag_temp IN (1, 2)", 
								  array($month, $year));

			if ($month == 12) {
				$id_akmt_periode = $this->ci->db->getField('id_akmt_periode',
														   'SELECT id_akmt_periode FROM akmt_periode WHERE flag_temp > 0 ORDER BY tahun DESC, bulan DESC');
				...
				foreach ($arr_saldo as $saldo) {
					...
					if (($saldo['code'] > 0) && ($saldo['code'] < 16)) {
						$arr_awal_dana[$saldo['id_akdd_detail_coa']] = $saldo['saldo'];
						$arr_dana[$saldo['id_akdd_detail_coa']] = $this->ci->db->getLastID('akmt_buku_besar_id_akmt_buku_besar_seq');
					}					
				}

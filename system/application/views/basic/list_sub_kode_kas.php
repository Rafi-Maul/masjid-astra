  		...
  		<div id='act-bar'>
  			<div class='non-print-act'>
				...
				<a title='Tambah' class='act-button' href="javascript: openPop('basic/subKodeKas/add', 'tambah');">
					<img src='<?php echo $img_folder; ?>/add.gif' border='0' alt='Tambah' align='absmiddle'/>Tambah
				</a>
				<?php } ?>
				<?php if ($s4b_password->getAccess('hapus')) { ?>
				<a title='Hapus' class='act-button' href="javascript: delForm('main-form', 'basic/subKodeKas/del');">
  			...
  			<div class='print-act'>
				...
				<a title='Laporan' class='act-button' href="javascript: openPop('basic/subKodeKas/report/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'report');">
  			...
  			<form id='main-form' method='post' action=''>
  				<table id='tbl' width='100%' border='0' cellpadding='3' cellspacing='0'>
  					...
  					<tbody>
  						<?php
  							$i = $s4b_paging['start'];
  							foreach ($arr_sub_kode_kas as $sub_kode_kas) {
  						?>
  						<tr>
  							<td class='td-no'><?php echo ++$i; ?>.</td>
  							<td class='td-check'>
								<input type='checkbox' name='id_list[]' value='<?php echo $sub_kode_kas['id_sub_kode_kas']; ?>'/>
							</td>
  							<td class='td-edit'>
  								<?php if ($s4b_password->getAccess('edit')) { ?>
							  	<a href="javascript: openPop('basic/subKodeKas/edit/<?php echo $sub_kode_kas['id_sub_kode_kas']; ?>/<?php echo $page; ?>/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'edit');"><img src='<?php echo $img_folder; ?>/edit.gif' border='0' alt='Edit'/></a>
							  	<?php } else { ?>
							  	&nbsp;
							  	<?php } ?>
							</td>
  							<td><?php echo $sub_kode_kas['kode_induk'] . '. ' . $sub_kode_kas['kas']; ?>&nbsp;</td>
  							<td><?php echo $sub_kode_kas['kode'] . '. ' . $sub_kode_kas['sub_kas']; ?>&nbsp;</td>  							
  							<td><?php echo ($sub_kode_kas['flag_in_out'] == 'i' ? 'Penerimaan' : 'Pengeluaran') ?></td>
  							<td><?php echo $sub_kode_kas['keterangan']; ?>&nbsp;</td>
  						</tr>  						
  						<?php
  							}  						
  						?>

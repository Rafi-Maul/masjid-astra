  		...
  		<div id='act-bar'>
  			<div class='non-print-act'>
				...
				<a title='Tambah' class='act-button' href="javascript: openPop('basic/rekeningBank/add', 'tambah');">
					<img src='<?php echo $img_folder; ?>/add.gif' border='0' alt='Tambah' align='absmiddle'/>Tambah
				</a>
				<?php } ?>
				<?php if ($s4b_password->getAccess('hapus')) { ?>
				<a title='Hapus' class='act-button' href="javascript: delForm('main-form', 'basic/rekeningBank/del');">
  			...
  			<div class='print-act'>
				...
				<a title='Laporan' class='act-button' href="javascript: openPop('basic/rekeningBank/report/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'report');">
			...
			<form name='search-form' action='<?php echo site_url($this->uri->uri_string()); ?>' method='post'>
				<table border='0' cellspacing='0' cellpadding='2'>
					<tr>
						...
						<td>
							<select name='filter'>
							  <option value='1' <?php if ($filter == 1) { ?>selected<?php } ?>>Bank</option>
							  <option value='2' <?php if ($filter == 2) { ?>selected<?php } ?>>No. Rekening</option>
							</select>
						</td>
						<td>
  			...
  			<form id='main-form' method='post' action=''>
  				<table id='tbl' width='100%' border='0' cellpadding='3' cellspacing='0'>
  					...
  					<tbody>
  						<?php
  							$i = $s4b_paging['start'];
  							foreach ($arr_rekening as $rekening) {
  						?>
  						<tr>
  							<td class='td-no'><?php echo ++$i; ?>.</td>
  							<td class='td-check'>
								<input type='checkbox' name='id_list[]' value='<?php echo $rekening['id_rekening_bank']; ?>'/>
							</td>
  							<td class='td-edit'>
  								<?php if ($s4b_password->getAccess('edit')) { ?>
							  	<a href="javascript: openPop('basic/rekeningBank/edit/<?php echo $rekening['id_rekening_bank']; ?>/<?php echo $page; ?>/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'edit');"><img src='<?php echo $img_folder; ?>/edit.gif' border='0' alt='Edit'/></a>
							  	<?php } else { ?>
							  	&nbsp;
							  	<?php } ?>
							</td>

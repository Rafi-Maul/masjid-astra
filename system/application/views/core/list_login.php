  		...
  		<div id='act-bar'>
  			<div class='non-print-act'>
				...
				<a class='act-button' href="javascript: openPop('core/login/add', 'tambah');">
					<img src='<?php echo $img_folder; ?>/add.gif' border='0' alt='Tambah' align='absmiddle'/>Tambah
				</a>
				<?php } ?>
				<?php if ($s4b_password->getAccess('hapus')) { ?>
				<a class='act-button' href="javascript: delForm('main-form', 'core/login/del');">
  			...
  			<div class='print-act'>
				...
				<a class='act-button' href="javascript: openPop('core/login/report/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'report');">
			...
			<form name='search-form' action='<?php echo site_url($this->uri->uri_string()); ?>' method='post'>
				<table border='0' cellspacing='0' cellpadding='2'>
					<tr>
						...
						<td>
							<select name='filter'>
							  <option value='1' <?php if ($filter == 1) { ?>selected<?php } ?>>Login</option>
							</select>
						</td>
						<td>
  				...
  				<table id='tbl' width='100%' border='0' cellpadding='3' cellspacing='0'>
  					...
  					<tbody>
  						...
  						<tr>
  							...
  							<td class='td-check'>
  								<?php if (!$login['flag_system']) { ?>
								<input type='checkbox' name='id_list[]' value='<?php echo $login['id_dd_users']; ?>'/>
								<?php } else { ?>
								&nbsp;
								<?php } ?>
							</td>
  							<td class='td-edit'>
  								<?php if (($s4b_password->getAccess('edit')) && (!$login['flag_system'])) { ?>
							  	<a href="javascript: openPop('core/login/edit/<?php echo $login['id_dd_users']; ?>/<?php echo $page; ?>/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'edit');"><img src='<?php echo $img_folder; ?>/edit.gif' border='0' alt='Edit'/></a>
							  	<?php } else { ?>
							  	&nbsp;
							  	<?php } ?>
							</td>
							<td><?php echo $login['group_name']; ?>&nbsp;</td>
  							<td><?php echo $login['username']; ?>&nbsp;</td>
  							<td><?php echo $login['note']; ?>&nbsp;</td>
  						</tr>  						
  						<?php
  							}  							
  						?>

  		...
  		<div id='act-bar'>
  			<div class='non-print-act'>
				...
				<a class='act-button' href="javascript: openPop('core/menu/add', 'tambah');">
					<img src='<?php echo $img_folder; ?>/add.gif' border='0' alt='Tambah' align='absmiddle'/>Tambah
				</a>
				<?php } ?>
				<?php if ($s4b_password->getAccess('hapus')) { ?>
				<a class='act-button' href="javascript: delForm('main-form', 'core/menu/del');">
  			...
  			<div class='print-act'>
				...
				<a class='act-button' href="javascript: openPop('core/menu/report/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'report');">
			...
			<form name='search-form' action='<?php echo site_url($this->uri->uri_string()); ?>' method='post'>
				<table border='0' cellspacing='0' cellpadding='2'>
					<tr>
						...
						<td>
							<select name='filter'>
							  <option value='1' <?php if ($filter == 1) { ?>selected<?php } ?>>Menu</option>
							  <option value='2' <?php if ($filter == 2) { ?>selected<?php } ?>>Modul</option>
							</select>
						</td>
						<td>
  				...
  				<table id='tbl' width='100%' border='0' cellpadding='3' cellspacing='0'>
  					...
  					<tbody>
  						...
  						<tr>
  							...
  							<td class='td-check'>
								<?php if (!is_numeric($menu['id_dd_sub_menus'])) { ?>
								<input type='checkbox' name='id_list[]' value='<?php echo $menu['id_dd_menus']; ?>'/>
								<?php } else { ?>
								&nbsp;
								<?php } ?>
							</td>
  							<td class='td-edit'>
							  	...
							  	<a href="javascript: openPop('core/menu/edit/<?php echo $menu['id_dd_menus']; ?>/<?php echo $page; ?>/<?php echo $filter; ?>/<?php echo $keyword; ?>', 'edit');"><img src='<?php echo $img_folder; ?>/edit.gif' border='0' alt='Edit'/></a>
							  	<?php } else { ?>
							  	&nbsp;
							  	<?php } ?>
							</td>
  							<td><?php echo $menu['modul']; ?>&nbsp;</td>
  							<td><?php echo $menu['menu']; ?>&nbsp;</td>  							
  							<td align='right'><?php echo $menu['order_number']; ?>&nbsp;</td>
  							<td><?php echo $menu['note']; ?>&nbsp;</td>
  						</tr>  						
  						<?php
  							}  						
  						?>

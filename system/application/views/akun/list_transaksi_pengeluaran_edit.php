		...
		<!-- BEGIN : Process Form -->
		<div id='process-form'>
			<form id='main-form' name='main-form' method='post' action='akun/transaksiPengeluaran/editAct'>
				<table id='tbl-top' border='0' cellpadding='0' cellspacing='0'>
					<tbody>
						...
						<tr>
							...
							<td class='input'>
								<select name='id_kode_kas' class='required' onchange='javascript: rubahKas(this);' style='width: 200px;'>
									...
									<option value='<?php echo $kas['id_kode_kas']; ?>' <?php if ($transaksi['id_kode_kas'] == $kas['id_kode_kas']) { ?>selected<?php } ?>><?php echo $kas['kas']; ?></option>
									<?php } ?>
								</select>
							</td>
						</tr>			
						<tr>
							...
							<td class='input'>
								<select name='id_sub_kode_kas' class='required' onchange='javascript: rubahSubKas(this);' style='width: 200px;'>
									...
									<option value='<?php echo $sub_kas['id_sub_kode_kas']; ?>' <?php if ($transaksi['id_sub_kode_kas'] == $sub_kas['id_sub_kode_kas']) { ?>selected<?php } ?>><?php echo $sub_kas['sub_kas']; ?></option>
									<?php } ?>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td class='input'>
								<select name='id_jenis_transaksi' class='required' onchange='javascript: rubahJenisTransaksi(this);' style='width: 200px;'>
									<option value=''>-- Pilih Transaksi --</option>
									<?php foreach ($arr_jenis as $jenis) { ?>
									<option value='<?php echo $jenis['id_jenis_transaksi']; ?>' <?php if ($transaksi['id_jenis_transaksi'] == $jenis['id_jenis_transaksi']) { ?>selected<?php } ?>><?php echo $jenis['transaksi']; ?></option>
									<?php } ?>
								</select>
							</td>
						</tr>
						<tr>
							<td class='label'>Dibayar Kepada</td>
							<td class='input'>
								<select name='id_pihak_penerima' class='required' style='width: 200px;'>
									<option value=''>-- Pilih Pihak Penerima --</option>
									<?php foreach ($arr_penerima as $penerima) { ?>
									<option value='<?php echo $penerima['id_pihak_penerima']; ?>' <?php if ($penerima['id_pihak_penerima'] == $transaksi['id_pihak_penerima']) { ?>selected<?php } ?>><?php echo $penerima['nama']; ?></option>
									<?php } ?>
								</select>
							</td>
						</tr>						
						<tr>
							...
							<td class='input'><input name='petugas' type='text' class='required' value="<?php echo $transaksi['petugas']; ?>" style='width: 200px;'/></td>
						</tr>												
						<tr>
						...
						<tr>
							<td class='label' colspan='2'>
								...
								<input type='hidden' name='id_mapping_penerima' value='<?php echo $transaksi['id_mapping_penerima']; ?>'/>
		...
		<script language='javascript' type='text/javascript'>
			...
			function rubahKas(elm) {
				...
				removeOptions(oForm.id_pihak_penerima, '');
			...
			function rubahSubKas(elm) {
				...
				removeOptions(oForm.id_jenis_transaksi, '');
				removeOptions(oForm.id_pihak_penerima, '');
				changeOptions(elm, oForm.id_jenis_transaksi, 'trans.jenis_transaksi_mapping_v', 'transaksi', 'id_jenis_transaksi', 'ORDER BY transaksi', false);						
			}
			
			function rubahJenisTransaksi(elm) {
				var oForm;
				oForm = elm.form;				
				removeOptions(oForm.id_pihak_penerima, '');
				changeOptions(elm, oForm.id_pihak_penerima, 'trans.jenis_transaksi_pihak_penerima_v', 'pihak_penerima', 'id_pihak_penerima', 'ORDER BY pihak_penerima', false);
			}
			//-->
		</script>
		<!-- END : Process Form -->
